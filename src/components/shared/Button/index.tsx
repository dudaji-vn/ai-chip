...
interface Props {
    type?: 'primary' | 'secondary' | 'danger' | 'cancel' | 'cancel_primary',
    className?: string,
...
export default function Button({ type = 'primary', className, children, ...props }: Props) {
    const types = {
        ...
        danger: 'py-[10px] px-5 text-center bg-red-700 text-white text-sm border border-red-700 rounded-sm hover:opacity-90 transition-all leading-none',
        cancel: 'py-[10px] px-5 text-center bg-gray-600 text-sm text-gray-400 border border-gray-500 rounded-sm hover:opacity-90 transition-all leading-none',
