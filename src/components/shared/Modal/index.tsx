...
export default memo(function Modal({ title, isOpen, onClose, className, children } : Props) {
    return (
        <Fragment>
            ...
            <div className={
                twMerge('fixed top-[50%] max-w-[640px] w-[95%] left-[50%] -translate-x-1/2 -translate-y-1/2 z-30 bg-gray-800 p-6 transition',
                isOpen ? 'opacity-100' : 'pointer-events-none opacity-0 -translate-y-1/3',
                className )}>
                <div className={twMerge('flex justify-end items-center pb-6',
                title ? 'relative after:content-[""] after:absolute after:bottom-0 after:h-[1px] after:-left-6 after:-right-6 after:bg-gray-700 ' : '',)}>
